...
import Subheader from './Theme/SubHeader/subheader'
    ...
    useEffect(() => {
        const getDestinations = async () => {
            ...
            for(let i=0;i<resDes.data.length;i++) {
                if(resDes.data[i].theme === themeName) {
                            ...
                            mydata.map(
                                (item,index) => (
                                                <div className="thm-col-md-3" key={index}>
                                                    <Card img={item.img===""?"(-)":item.img} title={item.name} subtitle={item.places===undefined ?"(-)":item.places.split('-')} ref={item.ref}/>
                                                </div>
                                            )
                                        )
                        }
